...
const InventoryTab: React.FC = () => {
  ...
  const rows: InventoryRow[] = useMemo(() => {
    ...
    return mockInventory.map((item) => {
      ...
      const isExpiringSoon =
        expDate && !isExpired
          ? expDate.getTime() - now.getTime() < 7 * 24 * 60 * 60 * 1000
          : false;
  ...
  const filteredRows = useMemo(() => {
    ...
    const byText = rows.filter((row) =>
      [row.itemName, row.location, row.notes ?? ''].some(
      ...
      render: (row) => (
        <Box>
          <Tooltip title="Edit">
            <IconButton
              size="small"
              onClick={() => alert(`Edit item: ${row.itemName}`)}
            >
  ...
  return (
    <Box>
        ...
        selectFilters={[
          {
            ...
            value: statusFilter,
            onChange: (val) =>
              setStatusFilter(val as 'all' | 'expired' | 'soon' | 'fresh'),
            options: [
              { value: 'all', label: 'All' },
      ...
      <DataTable
        ...
        renderCard={(row, { rowProps }) => {
          const status = row.isExpired
            ? { label: 'Expired', color: 'error' as const }
            : row.isExpiringSoon
              ? { label: 'Expiring soon', color: 'warning' as const }
              : { label: 'Fresh', color: 'success' as const };
          ...
          return (
            <RowCard
              ...
              details={[
                ...
                { label: 'Expiration', value: row.expirationDate || '-' },
